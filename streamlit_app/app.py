...
.risk-high,.risk-medium,.risk-low,.decision-sell,.decision-wait{color:white;border-radius:20px;padding:23px}.risk-high,.decision-sell{background:linear-gradient(135deg,#451418,#821f28);border:1px solid #b83a45}.risk-high{animation:glow 2.5s infinite}.risk-medium{background:linear-gradient(135deg,#44330b,#74570d);border:1px solid #a17b20}.risk-low,.decision-wait{background:linear-gradient(135deg,#0c3920,#176637);border:1px solid #287d47}.risk-title{font-size:1.55rem;font-weight:800}.decision-title{font-size:1.8rem;font-weight:800}.risk-sub{margin-top:6px;opacity:.9}
...
[data-baseweb="popover"],[role="listbox"]{background:#0b1c12!important;color:#e8f5ea!important}.stDataFrame div{background-color:#0e2115!important;color:#e8f5ea!important}
...
@st.cache_resource
def load_model():
    return joblib.load(APP_DIR / "crash_model_lite.pkl"), joblib.load(APP_DIR / "model_features.pkl")
...
st.markdown('<div class="hero"><span class="pill">● ML MARKET INTELLIGENCE</span><h1>🌾 Crop Price Crash Predictor</h1><p>Indian mandi price intelligence with historical crash-risk analysis and a sell-or-wait decision signal.</p><span class="leaf one">🌿</span><span class="leaf two">🍃</span><span class="leaf three">🌱</span></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if crop in TRAINED:
        ...
        st.info("Trend analysis mode")
    st.caption(f"Decision threshold: {THRESHOLD:.2f} · 15%+ drop / 7 days")
            ...
            if not r.empty:
                ...
                if missing:
                    ...
                    if probability>=THRESHOLD:
                        risk,cl,icon,msg="HIGH","risk-high","🚨","Historical patterns indicate elevated crash risk."
                    elif probability>=.40:
                        risk,cl,icon,msg="MEDIUM","risk-medium","⚠️","Price should be monitored closely before waiting."
                    else:
                        risk,cl,icon,msg="LOW","risk-low","✅","Historical patterns show lower short-term crash risk."
                    st.markdown(f'<div class="{cl}"><div class="risk-title">{icon} {risk} RISK</div><div class="risk-sub">Estimated probability of a 15%+ drop within 7 days: <b>{probability*100:.1f}%</b></div><div class="risk-sub">{msg}</div></div>',unsafe_allow_html=True)
                    if crash:
                        st.markdown('<div class="decision-sell"><div class="decision-title">🔴 SELL NOW</div><div class="risk-sub">The model detects a higher probability of a significant short-term price fall. Selling now may reduce exposure to the predicted crash.</div></div>',unsafe_allow_html=True)
                    else:
                        st.markdown('<div class="decision-wait"><div class="decision-title">🟢 SAFE TO WAIT</div><div class="risk-sub">The model does not currently cross the crash threshold. Historical patterns suggest lower short-term crash risk, so waiting may be reasonable.</div></div>',unsafe_allow_html=True)
                    ...
                    st.caption("Decision is based on the trained model's historical price-pattern probability, not a guaranteed future price.")
        else:
            st.markdown('<div class="panel"><h3>📊 Trend Mode</h3><p>The trained crash model currently supports Banana, Brinjal, Cabbage, Garlic and Green Chilli. Other crops remain available for price exploration.</p></div>',unsafe_allow_html=True)
...
with t3:
    ...
    st.write("The model uses price lags, percentage changes, rolling averages, volatility, moving-average relationships, month and day-of-week features.")
    st.info("SELL NOW is shown when the predicted crash probability reaches 55%. SAFE TO WAIT is shown below that threshold. This is a historical-pattern decision-support signal, not a guarantee.")
    st.markdown('</div>',unsafe_allow_html=True)
...
with t4:
    ...
    st.write("Crop Price Crash Predictor is a student ML decision-support application built from historical Agmarknet mandi prices. It estimates whether a significant short-term price fall is likely from historical patterns and converts that probability into a sell-or-wait signal.")
